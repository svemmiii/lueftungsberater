...
def _message(language: str | None, room: str, trigger: str) -> tuple[str, str]:
    ...
    messages = {
        "de": {
            ...
            NOTIFY_TRIGGER_ALL_CLEAR: f"Für {room} ist eine Entwarnung eingegangen. Die harte Schutzsperre ist aufgehoben; der Lüftungsassistent bewertet Raum- und Außenbedingungen wieder normal.",
        },
        "en": {
            NOTIFY_TRIGGER_AIRING_RECOMMENDED: f"Ventilating {room} is worthwhile again now.",
            ...
            NOTIFY_TRIGGER_WEATHER_DANGER: f"A window or door in {room} is still open while a severe-weather warning is active. Please check it and close it if needed.",
            ...
            NOTIFY_TRIGGER_ALL_CLEAR: f"An all-clear has been issued for {room}. The hard protection lock has been removed, and Fresh Air Assistant is evaluating the indoor and outdoor conditions normally again.",
        },
        "tr": {
            NOTIFY_TRIGGER_AIRING_RECOMMENDED: f"{room} odasını havalandırmak yeniden uygun hâle geldi.",
