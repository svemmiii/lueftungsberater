...
### CO₂-High-Load für stark belegte Wohnräume
- **Wiederholter schneller CO₂-Rebound ist jetzt ein eigener Betriebszustand:** Zwei schnelle Rückkehr-Ereignisse nach erfolgreich beendeten CO₂-Sitzungen innerhalb des 90-Minuten-Reboundfensters aktivieren weiterhin für zwei Stunden den High-Load-Kontext. Der Assistent interpretiert dies als anhaltend hohe CO₂-Last/Belegung, ohne eine Personenzahl zu schätzen.
- **Kein Alarm-Pingpong mehr bei 1400–1800 ppm:** Im aktiven High-Load-Zustand erzeugt eine normale Rückkehr unter 1800 ppm keine neue Lüftungsaufgabe. Die Raumluft-Perspektive bleibt bewusst gelb und erklärt die erkannte hohe Belegung; die Handlung bleibt ruhig/optional.
- **Trendbasierte Wiederfreigabe vor 1800 ppm:** Die CO₂-Steigung wird nur aus echten Messwertänderungen und mindestens eine Minute auseinanderliegenden Samples berechnet. Die Prognose wird erst nach **zwei aufeinanderfolgenden echten positiven Intervallen** freigegeben; ein einzelner Sprung kann damit keinen vorzeitigen High-Load-Alarm mehr auslösen. Ein fallendes Intervall setzt die Bestätigung zurück. Erst wenn der bestätigte Trend ab 1400 ppm voraussichtlich innerhalb von höchstens zehn Minuten 2000 ppm erreicht, wird die Lüftung bereits vor 1800 ppm wieder empfohlen.
- **2000-ppm-Override bleibt absolut:** Werte über 2000 ppm bleiben unabhängig vom High-Load-Zustand `co2_critical`; keine Ruhe-, Rebound- oder Komfortregel kann diese Eskalation unterdrücken. Die 1800-ppm-Schwelle ist ausdrücklich eine UX-Wiederfreigabe innerhalb des auffälligen Bereichs und kein medizinischer Grenzwert.
- **Wintertaugliche Sitzungsziele statt 850-ppm-Zwang:** Sobald im High-Load-Modus eine neue CO₂-Sitzung wirklich nötig ist, wird das Endziel abhängig von der Außentemperatur nach oben begrenzt: bei ≤5 °C mindestens 1300 ppm, unter 12 °C mindestens 1250 ppm, unter 18 °C mindestens 1150 ppm und bei milderen Bedingungen mindestens 1000 ppm. Strengere Ziele aus ungünstigen Außenbedingungen bleiben erhalten; die Außentemperatur kann niemals den Start einer wegen hohen CO₂ nötigen Lüftung verhindern.
- **High-Load- und Trendgedächtnis ist restartfest:** Reboundstatus, Trendanker und geglättete Steigung werden zusammen mit dem vorhandenen CO₂-Sessionzustand gespeichert und nach einem Neustart wiederhergestellt.

### Session-/Direktstations-Hardening
- **CO₂-Session macht die Raumluft-Karte nicht mehr allein nervöser:** Eine milde CO₂-Situation, deren Raumluft-Perspektive bei geschlossenem Fenster grün ist, bleibt bei identischen Messwerten auch nach dem Öffnen grün. Die native Lüftungskarte darf weiterhin korrekt von `open_now` auf `keep_open` wechseln. Eine zuvor bereits starke CO₂-Lage (`co2_high`/`co2_critical`) behält dagegen ihre bestehende Dringlichkeit während der laufenden Session.
- **Live-Entity-ID-Renames der direkten ESPHome-Station:** Raum-Coordinatoren verfolgen jetzt Entity-Registry-Updates der aktuell gebundenen Direktstations-Sensoren, lösen das Geräte-Sensor-Set nach einer Umbenennung erneut auf und binden ihre State-Listener sofort neu. Erfolgreich aufgelöste neue Entity-IDs werden zusätzlich in der Stationskonfiguration persistiert, damit die Zuordnung auch bei einem später hinzugefügten gleichartigen Sensor erhalten bleibt. Ein Integrations-Reload ist dafür nicht mehr nötig.
- **Direkte ESPHome-Werte erhalten echte Freshness:** CO₂, Luftfeuchte **und Innentemperatur** laufen jetzt durch dieselbe `State.last_reported`-Prüfung. Bleibt ein numerischer Sensorzustand länger als 180 Sekunden ohne neuen Report stehen, wird er nicht mehr in die Engine eingespeist; die Temperatur wird erst danach in °C normalisiert. Ein 30-Sekunden-Check aktualisiert den Raum auch ohne State-Change.
- **Zusätzlicher Sensor legt eine bereits konfigurierte Direktstation nicht lahm:** Wird später z. B. ein DS18B20 ergänzt und die automatische Geräteerkennung dadurch mehrdeutig, bleibt die zuvor gespeicherte CO₂-/Temperatur-/Feuchte-Zuordnung aktiv, solange die drei Entities weiterhin zum selben Gerät und zu den erwarteten Geräteklassen gehören.
- **Master↔HA-Korrelation vorbereitet:** Der Hardware-Report akzeptiert optionale `round_id` und `request_id` und gibt beide im Antwortobjekt unverändert zurück. Damit kann die spätere Master-Firmware eine HA-Antwort eindeutig ihrer Abfragerunde und Anfrage zuordnen.
- **Master-Rohwerte werden bereits beim Eingang plausibilisiert:** CO₂, Temperatur und Luftfeuchte außerhalb breiter physikalischer Wertebereiche werden als ungültig verworfen, sodass auch Hardware-Diagnose-Entities keine offensichtlichen Protokoll-/Sentinelwerte anzeigen.
- **Regressionstest ergänzt:** Der konkrete Fall „1200 ppm, Raumluft grün, Fenster öffnet, CO₂-Session startet“ ist jetzt explizit gegen Grün→Orange abgesichert.

### Direkte ESPHome-Lüftungsstation ohne Pflicht-Master
- **Zwei Verbindungsarten in derselben Stationsoberfläche:** Beim Hinzufügen einer Lüftungsstation wird jetzt zuerst `Direkt über ESPHome / Home Assistant` oder `Über Lüftungsstation-Master / ESP-NOW` gewählt. Ein einzelner ESP benötigt damit keinen künstlichen zweiten Master.
- **ESPHome-Gerät statt drei manueller Sensorwahlen:** Im Direktmodus listet der Lüftungsassistent geeignete bereits eingebundene ESPHome-Geräte und erkennt genau je einen CO₂-, Temperatur- und Luftfeuchtesensor automatisch. Danach wird nur noch der vorhandene Lüftungsassistent-Raum gewählt.
- **Direktstationen benutzen die Original-Entities:** Temperatur/rF/CO₂ werden direkt aus den vorhandenen ESPHome-Entities gelesen; deren normale Recorder-Historie bleibt die einzige Rohwert-Historie. Der Lüftungsassistent erzeugt dafür keine gespiegelten Hardware-Sensoren und kein doppeltes Geräteobjekt.
- **Entity-Renames robuster:** Die direkte Station speichert ihr ESPHome-Gerät und löst dessen eindeutiges CO₂-/Temperatur-/Feuchte-Set erneut über die Device Registry auf. Damit bleibt die Zuordnung bei einer normalen Entity-ID-Umbenennung funktionsfähig.
- **Verfügbarkeit statt Master-TTL:** Direkte ESPHome-Werte werden nur verwendet, wenn deren HA-Zustände aktuell verfügbar/numerisch sind. Die 180-s-Report-TTL bleibt ausschließlich für Master-/ESP-NOW-Stationen bestehen.
- **Master-Gerät nur bei Bedarf:** Reine Direkt-Setups erzeugen kein leeres `Lüftungsstation-Master`-Gerät. Der Hub wird erst bei einer Master-Station bzw. einer echten Master-Discovery angelegt; leere Master-Artefakte aus frühen v0.10.0-Builds werden beim Setup entfernt.
- **Master kann zugleich Raumstation sein:** Ein ESPHome-Gerät kann direkt einem Raum zugeordnet sein und später zusätzlich als Transport-Master für weitere ESP-NOW-Knoten dienen, ohne dass die Raumlogik dupliziert wird.

### Zweiter Nachaudit – Fensterkontakt, Feuchte-Timer und Stations-Freshness
- **Fensterkontakt ändert den Raumluft-Handlungsdruck nicht mehr allein:** Die Raumluft-Perspektive übernimmt bei einem geöffneten Fenster nicht mehr automatisch ein natives `keep_open` als stärkere Handlung. Bei identischen Messwerten bleibt die Raumfarbe deshalb stabil; nur die native Lüftungskarte darf ihre Formulierung passend zum Kontakt von „Lüften“ auf „Weiter lüften“ ändern. Grün bleibt auf der Raumluft-Karte immer ein echter No-Action-Zustand.
- **`humidity_observe` hat jetzt einen passenden Erklärungstext:** Eine bereits behandelte, aber weiterhin deutlich erhöhte Raumfeuchte wird gelb sichtbar beschrieben, ohne gleichzeitig fälschlich „kein relevanter Grund“ zu behaupten oder eine neue Lüftungspflicht auszulösen.
- **Feuchte-Rearm-Timer bleibt bis zum Ende der Mindest-Ruhe aktiv:** Sind +3 %-Punkte Verschlechterung oder deutlich bessere Außenbedingungen bereits 15 Minuten stabil, bevor die einstündige Mindest-Ruhe endet, bleibt jetzt die verbleibende Ruhezeit als nächster Check bestehen. Ein `0 s`-Zwischenwert kann den Coordinator-Timer nicht mehr versehentlich löschen.
- **Feuchte-Session startet nur noch aus eigener physikalischer Eignung:** CO₂ oder ein anderer unabhängiger Grund kann weiterhin das Fenster öffnen lassen, startet aber keine Feuchte-Session mehr, wenn die Außenluft absolut nicht trockener genug ist. Entscheidend ist jetzt ausschließlich das Trocknungspotenzial des Feuchtegrunds selbst.
- **Lüftungsstationen bekommen eine Freshness-/Offline-TTL:** Nach 180 Sekunden ohne neuen Report gilt eine Station als offline/stale. Ein 30-Sekunden-Wächter aktualisiert die HA-Entities aktiv; stale CO₂-/Temperatur-/Feuchtewerte werden nicht mehr in die Engine eingespeist und eine einem Raum zugeordnete Station fällt dabei nicht still auf möglicherweise alte manuell konfigurierte Raumsensoren zurück. Beim nächsten gültigen Report wird sie automatisch wieder online und nutzbar.
- **Gezielte Regressionstests ergänzt:** Kontaktwechsel bei Feuchte/CO₂/Temperatur, `humidity_observe`-Text, beide Feuchte-Resttimer, grundbezogene Feuchte-Aktionsfähigkeit sowie Hardware-Freshness/Offline-Markierung sind zusätzlich abgesichert.

### Nachaudit-Korrekturen und Lüftungsstationen in Home Assistant
- **Temperatur-Fensterkontakt vollständig repariert:** Die 0,5-K-Fortsetzungslogik ist jetzt auch in der eigentlichen Moduswahl vom reinen Kontaktzustand entkoppelt. Im schmalen Fortsetzungsband bleibt die Raumansicht bei identischen Messwerten auf beiden Seiten des Kontakts ruhig gelb statt beim Öffnen von grün auf orange zu springen.
- **Manuelles Schließen einer Feuchte-Session beendet den Versuch sauber:** Wird eine laufende Komfort-Feuchte-Lüftung vom Nutzer geschlossen, wird der erreichte Zustand als grundbezogene Ruhe-/Rearm-Referenz gespeichert. Derselbe Messwert kann dadurch nicht im nächsten Snapshot sofort wieder `open_now` erzeugen.
- **Hohe Feuchte wird während der Ruhephase nicht mehr unsichtbar:** 65–74 % rF können weiterhin als gelber Raumzustand sichtbar bleiben, obwohl der bereits bearbeitete Komfortgrund keine neue Fensteraufgabe auslöst. Ab 75 % bleibt die bestehende dringliche Reaktion erhalten.
- **Lüftungsstation ist jetzt ein echter Subentry-Typ derselben Integration:** Neben Räumen kann in einem lokalen Lüftungsassistenten jetzt eine `Lüftungsstation` angelegt und direkt einem vorhandenen Raum zugeordnet werden. Temperatur, Feuchte und CO₂ kommen dann automatisch aus der Station; diese drei Sensoren müssen im Raum nicht zusätzlich einzeln ausgewählt werden.
- **Raum kann für eine spätere Station bereits ohne lokale Temperatur-/Feuchtesensoren angelegt werden:** Solange keine Station bzw. keine direkten Sensoren Daten liefert, bleibt der normale Incomplete-Data-Zustand bestehen; es werden weiterhin keine Werte erfunden.
- **Physisches Geräte-Modell vorbereitet und aktiv:** Bei Nutzung des Master-/ESP-NOW-Wegs erzeugt der lokale Eintrag ein Master-/Hub-Gerät. Gekoppelte Master-Stationen erscheinen als eigene Geräte mit `via_device_id` zum Master und erhalten CO₂-, Temperatur-, Feuchte-, Online- sowie optionale Funkdiagnose-Entities mit normaler Recorder-Historie. Direkt angebundene ESPHome-Stationen verwenden dagegen bewusst ihr bereits vorhandenes ESPHome-Gerät und dessen Entities.
- **Authentifizierte Hardware-API eingebaut:** `/api/lueftungsberater/hardware/discover` nimmt JOIN-Kandidaten eines künftigen Masters entgegen; `/api/lueftungsberater/hardware/report` nimmt die Rohwerte einer zugeordneten Station an und liefert direkt den bereits von Home Assistant berechneten Raumstatus, die kurze Empfehlung und den gewählten Displaymodus zurück. Unbekannte Hardware darf keine Raumwerte einspeisen und wird nur als Kandidat vorgemerkt.
- **Wichtig zur Abgrenzung:** ESP-NOW-Paketformat, Mehrhop-Routing, Retries/ACKs und das eigentliche Master-/Node-C++-Programm bleiben Hardware-Firmware und sind nicht Teil des Home-Assistant-Python-Codes. Die Home-Assistant-Seite für Erkennung, Zuordnung, Rohwerte und Rückgabe der fertigen Anzeige ist jetzt jedoch vorhanden.

### Ruhigere, grundbezogene Session-Logik
- **Raumluft-Grün ist jetzt eine harte Invariante:** In der Raumluft-Perspektive bedeutet Grün immer, dass aktuell keine Handlung nötig ist. Kombinationen wie `green + keep_open`, `green + open_now`, `green + room_need` oder ein dazu widersprüchlicher Begründungstext werden technisch verhindert. Farbe, Handlung und Begründung werden nach der finalen UI-Korrektur aus demselben Zustand gerendert.
- **Fensterkontakt ändert keine Temperatur-Session mehr allein:** Die niedrigere Fortsetzungs-Hysterese einer bereits laufenden Temperatur-Sitzung hängt nicht mehr von `window_open` ab. Ein reiner Kontaktwechsel kann dadurch bei identischen Messwerten nicht mehr zwischen Start- und Fortsetzungsgrenze springen.
- **Feuchte-Sitzungen messen echten Fortschritt über absolute Feuchte:** Beim Start werden Zeit, absolute/relative Innenfeuchte und Innentemperatur gespeichert. Alle 15 Minuten wird geprüft, ob die beste absolute Innenfeuchte seit dem letzten Anker um mindestens 0,25 g/m³ gefallen ist. Bleibt der zusätzliche Effekt darunter, gilt die Sitzung als ausgeschöpft und ein offenes Fenster darf geschlossen werden.
- **Ausgeschöpfte Feuchte wird nicht nach einer starren Stunde erneut gestartet:** Eine Stunde ist nur die Mindest-Ruhezeit. Der Feuchtegrund bleibt danach disarmed, solange der Raum praktisch auf demselben Niveau bleibt. Ein Neustart entsteht erst nach einer echten Zustandsänderung.
- **Rearm über reale Verschlechterung statt Grenzwert-Flattern:** Nach einer abgeschlossenen Feuchte-Sitzung wird das erreichte Niveau gespeichert. Erst ungefähr +3 Prozentpunkte rF über diesem Referenzniveau, über 15 Minuten stabil und nach der Mindest-Ruhezeit, reaktivieren den normalen Feuchtegrund. Ein deutlich niedriger, stabiler Zustand kann die Session ebenfalls sauber neu scharfstellen.
- **Bessere Außenbedingungen sind zunächst nur eine Gelegenheit:** Maßgeblich ist das absolute Trocknungspotenzial `innen AH - außen AH`. Wird es gegenüber dem letzten Versuch um mindestens 1,0 g/m³ besser und bleibt das 15 Minuten stabil, entsteht nur eine gelbe optionale Gelegenheit – keine neue Pflicht zum Lüften.
- **Dusch-/Koch-Peaks bekommen Ereignis- und Erholungslogik:** Ein schneller Anstieg um etwa 6 Prozentpunkte rF innerhalb von 15 Minuten gilt als neue Situation und darf eine normale Feuchte-Ruhephase sofort durchbrechen. Nach deutlichem Rückgang wird die Erholung erkannt, damit ein fallender Peak nicht am 60-%-Grenzwert sofort erneut alarmiert.
- **Grundbezogene Zustände statt globalem Cooldown:** Feuchte-Rearm unterdrückt weder CO₂ noch Innenluft/Partikel, Schimmel-/Oberflächenrisiko oder Hard-Locks. Die Engine veröffentlicht intern alle gleichzeitig aktiven Gründe, damit Session-Gedächtnis auch dann korrekt weitergeführt wird, wenn ein stärkerer Grund die sichtbare Hauptentscheidung übernimmt.
- **CO₂-Rebound/High-Load wurde erweitert:** Der frühere reine Diagnosehinweis mit tieferem Folgeziel wurde durch den oben beschriebenen High-Load-Betriebszustand mit ruhiger 1400–1800-ppm-Zone, Trendprognose, >2000-ppm-Override und temperaturabhängigen Kurzlüftungszielen ersetzt.
- **Partikel-/Innenluftgründe bleiben unabhängig:** Die bestehende Baseline-/Trendbewertung und der Vergleich mit derselben Außenkomponente bleiben erhalten. Neue Feuchte-Ruhephasen können einen belastbaren PM/VOC/NO₂-Grund nicht unterdrücken.
- **Prioritäten bleiben eindeutig:** Hard-Lock/Außenwarnung gewinnt weiterhin vor kritischer Innenlage, diese vor normalen gesundheitlichen Gründen, Komfortgründen, optionalen Gelegenheiten und Routine. Bestehende dynamische CO₂-Ziele, Mindestlüftungsdauer, stärkster-Innenwert-Prinzip und Hysteresen bleiben erhalten.

### Tests und Vorbereitung der Hardware-Hub-Erweiterung
- **Neue gezielte Regressionstests:** Feuchte-Plateau, Ruhephase ohne Timer-Neustart, besseres Trocknungspotenzial, +3-%-Rearm, schneller Feuchte-Peak, Fensterkontakt/Temperatur-Hysterese, Raumluft-Farbe+Handlung+Text, Hard-Lock-Priorität, CO₂-Rebound und Grund-Unabhängigkeit sind abgedeckt.
- **Hub/Master bleibt Teil derselben Integration:** Die vorhandene Integration bleibt der einzige Ort für die Lüftungslogik und ist bereits als `hub` modelliert. Die spätere ESP-NOW-/WireGuard-Hardware wird als Transport-/Geräteschicht daran angebunden; es entsteht keine zweite Lüftungslogik oder zweite Benutzerintegration. Das tatsächliche ESP-NOW-Pairing/Mehrhop-Protokoll wird erst mit der Hardware-Firmware aktiviert und ist in v0.10.0 noch kein laufender Transportweg.
...
- **Raumluftfarbe bleibt eine eigene Perspektive, die konkrete Fensterhandlung widerspricht der Engine aber nicht mehr:** Bei offenem Fenster werden native `close_now`, `better_close` und `can_close` auch auf der sichtbaren Raumluftkarte respektiert. Ein natives `keep_open` kann umgekehrt nicht mehr als `can_close` erscheinen. Die bewusst gewünschte Abschwächung/Schwellenverschiebung der Raumluftfarben durch schlechte Außenbedingungen bleibt unverändert.
- **24-h-Routine nach erfüllten fünf Minuten:** Sobald die laufende Routine-Lüftung mindestens fünf **sicher beobachtete** Offen-Minuten erreicht hat oder die Außenlage inzwischen gegen weiteres Lüften spricht, zeigt die Raumluftkarte nicht mehr „Noch kurz weiterlüften“, sondern die passende Schließ-/Fertig-Handlung samt dazugehöriger Begründung.
- **Neue Räume bekommen sofort einen 24-h-Zeitanker:** Der Airing-Tracker speichert ab der ersten Initialisierung einen separaten `tracking_started_at`-Zeitpunkt. Damit kann die 24-h-Routine auch dann fällig werden, wenn noch nie eine bestätigte Lüftung stattgefunden hat. `last_confirmed_airing` bleibt dabei weiterhin `None`, bis eine mindestens fünf Minuten lange Sitzung tatsächlich beendet/geschlossen wurde.
- **`innenluft_abwaegung` vollständig als Trade-off gemappt:** Die native Empfehlung lautet nun wie bei den übrigen gelben Abwägungen `short_observation` statt geschlossen `wait` bzw. offen `can_close`. Dadurch bleibt auch die Automation-Schnittstelle semantisch konsistent mit `_TRADEOFF_MODES`.
- **CO₂-Ausfall während laufender Lüftung ohne falsches „Ziel erreicht“:** Fehlt der aktuelle CO₂-Wert und werden die Außenbedingungen gleichzeitig ungünstig, darf die Engine weiterhin zum Schließen raten, verwendet aber nicht mehr `lueftung_fertig`/`airing_finished`. Stattdessen erklärt ein eigener Grund, dass das CO₂-Ziel nicht verifiziert werden kann und das Schließen aus der aktuellen Außenlage folgt.
- **Fensterkontakt-Entity-IDs robuster:** Der Airing-Tracker folgt Home-Assistant-Entity-Registry-Umbenennungen (`old_entity_id` → neue Entity-ID) während der Laufzeit und persistiert die Auflösung über Neustarts. Die Registry-Identität wird jetzt ebenfalls persistent gespeichert und enthält ausdrücklich die Entity-Domain; dadurch kann ein `binary_sensor` niemals nur wegen gleicher Platform/Unique-ID auf einen `sensor` umgebogen werden. Wenn ein Kontakt während eines HA-Neustarts gelöscht/neuerstellt wurde und die neue Entity beim Start bereits existiert, sucht der Tracker einmalig nach genau einem eindeutigen Identitätstreffer und übernimmt ihn sicher.
- **Diagnose zeigt den tatsächlich verfolgten Fensterkontakt:** `source_window_entities` verwendet die aktuell vom Airing-Tracker aufgelösten Entity-IDs statt nach Rename/Recreate weiterhin die veralteten IDs aus der ursprünglichen Subentry-Konfiguration anzuzeigen.
- **Laufende Routinelüftung wird auch sprachlich als laufend erkannt:** Solange das Fenster offen ist, verschwindet die irreführende Begründung „Seit X Stunden wurde keine bestätigte Lüftung erkannt“. Unter ungünstigen Außenbedingungen beschreibt die Karte stattdessen die bereits laufende Öffnung; nach mindestens fünf Minuten ausdrücklich, dass die kurze Luftaustauschdauer bereits erreicht wurde und die Lüftung jetzt beendet werden sollte. `last_confirmed_airing` bleibt korrekt erst nach dem tatsächlichen Schließen historisch bestätigt.
- **Fünf-Minuten-Failsafe als eigener Live-Latch:** Eine laufende Fensteröffnung wird nach fünf *sicher beobachteten* Minuten genau einmal als `current_airing_qualified` markiert. Damit gilt eine überfällige 24-h-Routine für die aktuelle Entscheidung ab diesem Moment als erfüllt, ohne `last_confirmed_airing` vorzeitig zu verändern. Bleibt das Fenster danach noch Stunden offen, wandert der historische Zeitpunkt nicht mit; erst das tatsächliche Schließen setzt `last_confirmed_airing` auf den Schließzeitpunkt. Ein anderer aktiver Grund wie CO₂, Feuchte oder Temperatur kann das Weiterlüften natürlich weiterhin verlangen.
- **Latch bleibt restart- und Unknown-sicher:** `minimum_reached_at` wird persistent gespeichert. Ein bereits qualifizierter Lauf überlebt kurze `unknown`/`unavailable`-Phasen und HA-Neustarts. Eine Sitzung, die die Fünf-Minuten-Grenze erstmals während eines unbekannten Kontaktzustands überschreitet, wird dagegen nicht allein durch diese unbekannte Zeit qualifiziert; erst ein wieder eindeutig `on` meldender Kontakt darf den bereits überschrittenen Meilenstein setzen.
- **Kein erfundener Abschluss nach langem Fensterkontakt-Ausfall:** Läuft `unknown`/`unavailable` länger als die zweiminütige Grace-Phase, beendet der Tracker die aktuelle Live-Sitzung konservativ, setzt aber **nicht** `last_confirmed_airing`. Ein historischer bestätigter Lüftungszeitpunkt entsteht nur bei einem tatsächlich beobachteten `off`. War die Sitzung vor dem Kontaktverlust bereits fünf Minuten sicher qualifiziert, speichert der Tracker stattdessen separat `last_qualified_airing_seen_at` am letzten sicher bekannten Offen-Zeitpunkt. Dieser schwächere, persistente Routine-Anker verhindert, dass die alte 24-h-Routine unmittelbar wieder auftaucht, ohne einen Schließzeitpunkt zu erfinden. Ein später sauber beobachteter qualifizierter `off` ersetzt diesen Fallback wieder durch den echten `last_confirmed_airing`-Zeitpunkt. Unknown-Zeit allein kann die Mindestdauer weiterhin nicht erfüllen.
- **Konkrete Außengründe haben Textvorrang:** Ist eine überfällige Routine durch die aktuelle Fünf-Minuten-Sitzung bereits erfüllt und ein konkreter Außengrund verlangt das Schließen, zeigt die Karte wieder diesen spezifischen Grund (z. B. sehr schlechte Außenluft, Wettervorsicht, außen zu kalt/feucht) statt ihn mit `routine_open_sufficient_close` zu überdecken. Die Entscheidung selbst ändert sich dadurch nicht.
- **Regressionstests erweitert:** Neue Engine-Tests decken die offenen-Fenster-Konflikte, Routineabschluss, die laufende Routine-Semantik, CO₂-Ausfall-Semantik und das Indoor-Air-Trade-off-Mapping ab. Airing-Tests decken den Zeitanker, Domain-Schutz, persistierte Registry-Identitäten, Recreate-Reparatur über einen Neustart sowie den qualifizierten `unknown`/`unavailable`-Abbruch ohne erfundenen Close und mit erhaltenem Routine-Anker ab.

## v0.9.5

### Zweite Release-Hardening-Runde nach 0.9.5-Regressionsaudit
- **24-h-Routinelüftung endet wieder nach fünf realen Offen-Minuten:** Der allgemeine `independent_keep_open`-Schutz behält konkrete beneficial Indoor-Gründe, behandelt die zeitbegrenzte Routine aber nicht mehr als unbegrenzten Fortsetzungsgrund. Der CO₂+Innenluft-Fix bleibt dabei erhalten.
- **Temperatur-Dauertext wieder physikalisch korrekt:** Eine reine Temperatur-Sitzung zählt ihren eigenen Temperaturgrund nicht mehr als zusätzlichen unabhängigen Zielgrund. Wenn Außenluft nur bis zu ihrer eigenen Temperatur helfen kann, zeigt die Karte wieder `while_temperature_helps` statt ein unerreichbares „bis die Ziele erreicht sind“ zu versprechen.
- **O₃-Parts fließen wirklich in die Gesamtluftqualität ein:** Die Provider-/Lokal-Zusammenführung verwendet für bekannte physikalische Schadstoffe jetzt die gemeinsame `classify_absolute()`-Klassifikation. Dadurch werden bereits korrekt normalisierte `o3_parts`-Werte in `ppb` nicht mehr beim anschließenden Gesamt-LQI verworfen.
- **Persistente Live-Wetter-Safety folgt der tatsächlichen Gefahrenquelle:** Harte Gefahren speichern die konkrete(n) Entity/Entities, die den Lock ausgelöst haben. Eine lokale Böe bleibt damit auch dann fail-safe, wenn `weather.*` unabhängig davon verfügbar oder unverfügbar ist; CLEAR gilt erst, wenn die tatsächliche Gefahrenquelle wieder verfügbar und sicher ist.
- **NINA-Stale-TTL wird nicht mehr verdoppelt:** Ein aus gecachten NINA-Details abgeleiteter Hard-Lock trägt den Zeitstempel der letzten erfolgreichen Provider-Evidenz in den persistenten Safety-State. Wiederverwendung alter Details startet die zusätzliche 60-Minuten-Uhr nicht neu.
- **Böen-Regressions­test auf echten Config-Key korrigiert:** Der Test verwendet `CONF_OUTDOOR_GUST` / `outdoor_wind_gust` statt des nicht existierenden `outdoor_gust`-Keys. Neue Regressionstests sichern zusätzlich die konkrete Wetter-Gefahrenquelle und die nicht addierbaren NINA-TTLs ab.

### Release-Hardening nach erweitertem Sensor-/CO₂-Audit
- **Offene CO₂-Sitzungen + Innenluft:** Ein weiterhin sinnvoller unabhängiger Innenluft-Grund kann im allgemeinen Open-Window-Postprocessing nicht mehr versehentlich zu `lueftung_fertig` überschrieben werden. Die Fortsetzung basiert jetzt auf dem bereits vollständig gegen die Außenlage geprüften beneficial-Signal und ist damit auch für künftige Indoor-Gründe robuster.
- **Lokale Wetterwerte bleiben unabhängig:** Lokaler Wind, Böen und Regen werden auch ausgewertet, wenn die gewählte Weather-Entity vorübergehend `unavailable` ist. Eine lokale 110-km/h-Böe kann die Fenstersperre deshalb weiterhin korrekt auslösen.
- **Gesamtluftqualität bleibt maßgeblich:** Ein direkt besserer PM-/VOC-/NO₂-Wert draußen darf einen anderen bekannten schlechten Außenschadstoff nicht mehr verdecken. Der direkte Vergleich bleibt ein Pro-Lüften-Signal, bekannte mäßige/schlechte Gesamtluft erzeugt aber eine echte Abwägung; sehr schlechte Außenluft bleibt restriktiv.
- **Ozon in `ppb`/`ppm`:** Lokale O₃-Sensoren werden wie andere physikalische Gassensoren in Masse vs. Parts getrennt normalisiert. `ppm` wird sicher zu `ppb` normalisiert; Ozon-Parts werden substanzspezifisch bewertet und niemals als Herstellerindex geraten.
- **Regen semantisch gehärtet:** Der ConfigFlow bietet bei numerischen Sensoren nur `precipitation_intensity` an. Bestehende Konfigurationen werden zur Laufzeit ebenfalls geprüft: akkumulierte Tages-/Gesamtsummen lösen kein dauerhaftes „regnet gerade“ mehr aus. Binary-Regensensoren werden nach Verfügbarkeit/`on`/`off` bewertet und nicht nach 30 Minuten unverändertem Zustand künstlich verworfen.
- **Luftqualitäts-Details vollständig:** Lokale Außen-Indizes/Parts bleiben in den Kartenattributen sichtbar. VOC und NO₂ wählen Innen- und Außen-Messart jetzt unabhängig, sodass z. B. innen `Index 7` und außen `350 µg/m³` gleichzeitig erscheinen können. Ozon wird ebenfalls in den Details angezeigt.
- **Klickquelle entspricht dem angezeigten Wert:** Für PM2,5/PM10/VOC/NO₂/O₃ wird die tatsächlich verwendete Provider- oder lokale Entity pro Schadstoff gespeichert. Fällt ein lokaler Sensor aus und greift der Provider-Fallback, öffnet ein Klick nicht mehr die ausgefallene lokale Entity.
- **AQ-Store-Bereinigung:** Beim Löschen eines Raums werden dessen eingebettete `room:<subentry_id>`-Baselines aus dem gemeinsamen Air-Quality-Store entfernt, ohne Standort-/Außenlernen anzutasten.

### Lokale Außenstation und erweiterte Innenluft-Sensorik
- **Lokale Außenstation optional:** Eigene Sensoren für Außentemperatur/-feuchte, Wind, Böen, Regen, PM2,5, PM10, VOC-/NO₂-Werte (Index oder Rohwert), Ozon und Außen-CO₂ können einzeln hinterlegt werden. Jeder Wert wird separat auf Verfügbarkeit, Alter, Einheit und physikalische Plausibilität geprüft. Nur ein gültiger lokaler Wert ersetzt die jeweilige offizielle Quelle; fehlt er, fällt genau diese Messgröße automatisch auf Wetter-/Luftqualitätsdaten zurück.
- **Keine Pflichtsensoren und keine erfundenen Werte:** Temperatur und relative Feuchte im Raum bleiben die einzigen Pflichtwerte. Alle neuen Luftqualitäts- und Außenstationssensoren sind optional; `unknown`/`unavailable`, veraltete oder unplausible Werte werden ignoriert statt geschätzt. Ohne neue Sensoren bleibt der bisherige Entscheidungsweg unverändert.
- **Innenluft-Sensoren passend zum Dyson BP04:** Optional unterstützt werden PM2,5, PM10, VOC, NO₂/NOx und Formaldehyd zusätzlich zum bereits vorhandenen CO₂-, Temperatur- und Feuchtepfad.
- **VOC/NO₂ erkennt Index oder Rohwert automatisch:** Dieselben optionalen Sensorfelder akzeptieren sowohl herstellerspezifische Indizes (z. B. Dyson) als auch echte physikalische Rohwerte. `µg/m³`/`mg/m³` werden als Massenkonzentration erkannt; `ppm`/`ppb` als Parts-Wert; einheitenlose bzw. explizite `index`-Werte als Herstellerindex. Ein Sensor mit physikalischer Device-Class aber fehlender/ungültiger Einheit wird bewusst verworfen statt als Index geraten.
- **Nur sichere Umrechnungen:** `mg/m³ ↔ µg/m³` und `ppm ↔ ppb` werden innerhalb derselben physikalischen Dimension normalisiert. Es gibt keine erfundene Umrechnung eines Dyson-/Herstellerindex in µg/m³ und keine pauschale VOC-Umrechnung zwischen Parts und Masse.
- **Absolute vs. relative Luftqualität getrennt:** PM2,5/PM10, Formaldehyd sowie VOC-/NO₂-Massenkonzentrationen werden mit passenden absoluten Bändern bewertet. Physikalisches NO₂ in `ppb/ppm` wird in `ppb` normalisiert und substanzspezifisch bewertet. Generische VOC-Parts-Werte und proprietäre VOC-/NO₂-Indizes lernen stattdessen eine eigene, persistente Raum-/Außen-Baseline und beeinflussen die Entscheidung erst mit ausreichend Historie bei klarer Abweichung bzw. Trend.
- **Innen/außen bleibt skalenfest:** Direkte Rohwertvergleiche werden nur vorgenommen, wenn innen und außen dieselbe normalisierte physikalische Größe besitzen (z. B. PM2,5, VOC in µg/m³ oder NO₂ in ppb). Unterschiedliche Messarten werden nie numerisch gegeneinander gerechnet, sondern erst nach ihrer jeweiligen Bewertung als Luftqualitätsstufe verglichen.
- **Innen-vs.-Außen-Abwägung:** Bei messbar belasteter Innenluft wird Lüften nur dann aktiv empfohlen, wenn die Außenluft für den relevanten Schadstoff bzw. die Gesamtluftqualität tatsächlich günstiger ist. Ist außen schlechter, bleibt das Fenster eher geschlossen; bei unbekannter Außenluft wird bewusst keine saubere Außenluft erfunden, sondern eine vorsichtige Abwägung angezeigt.
- **Safety-Hierarchie bleibt autoritativ:** Harte amtliche Schutzanweisungen und Wettergefahren bleiben über allen Komfort-/Luftqualitätsgründen. Danach werden konkrete gesundheitlich relevante Innenbedarfe gemeinsam mit CO₂, Feuchte, Schimmel-/Oberflächenrisiko und Temperatur nach dem bestehenden „stärkster Innenwert gewinnt“-Prinzip bewertet.
- **Kompakte Karte erweitert:** In der ausgeklappten Raumkarte werden konfigurierte Innen-/Außenwerte für PM2,5/PM10 sowie VOC/NO₂/Formaldehyd mit der tatsächlich erkannten Einheit angezeigt. Ein Index bekommt keine erfundene Einheit; Rohwerte werden als `µg/m³` bzw. `ppb` dargestellt. Nicht konfigurierte Messwerte erscheinen nicht.
- **Rückwärtskompatibilität geprüft:** 20.000 identische Zufallsfälle ohne neue Sensoren lieferten zwischen dem finalen v0.9.4-Enginepfad und v0.9.5 dieselben Farben, Modi, Empfehlungen und Primärgründe.

## v0.9.4

### CO₂-Session-Priorität und Ausfall-Härtung
- **Eine abgeschlossene oder fast abgeschlossene CO₂-Sitzung darf andere Lüftungsgründe nicht mehr abschwächen:** `co2_finish_ready` entfernt nur den CO₂-Bedarf. Wenn Temperatur, Feuchte, Oberflächenfeuchte/Schimmel oder ein anderer unabhängiger Grund weiterhin sinnvoll vom offenen Fenster profitiert, bleibt die Gesamtentscheidung `keep_open` statt auf `short_observation` oder `can_close` zu springen. Das gilt auch bei 1250-ppm-Sitzungen, bei denen CO₂ selbst noch als aktiver Bandwert vorhanden sein kann.
- **CO₂-Sensor-Ausfall während einer laufenden Sitzung:** Ist die fünfminütige Mindestphase bereits vorbei und der aktuelle CO₂-Wert vorübergehend unbekannt, wird das niemals als Zielerreichung interpretiert. Ohne anderen stärkeren Lüftungsgrund zeigt die Engine Gelb/`short_observation` mit dem Hinweis, dass das Sitzungsziel aktuell nicht bestätigt werden kann; ein unabhängiger `keep_open`-Grund bleibt dagegen vollständig autoritativ.
- Die Raumansicht darf eine CO₂-Sitzung nur dann zu Grün/„Lüften kann beendet werden“ machen, wenn die **Gesamtentscheidung selbst** tatsächlich `lueftung_fertig` ist. Ein bloßes `co2_finish_ready=True` reicht nicht mehr, solange ein anderer Lüftungsgrund weiter besteht.
- Neue Regressionstests sichern Zielnähe und Abschluss bei parallelem Temperatur-/Feuchtebedarf, einen 1250-ppm-Zielpfad mit gleichzeitiger Temperaturanforderung sowie `co2=None` nach der Mindestphase. Die isolierbare Kernmatrix umfasst damit 222 grüne Engine-/Nacht-/CO₂-/Localization-Tests.
- **Recorder-Orphan-Retry wird persistent:** Fehlgeschlagene Purges bereits gelöschter ConfigEntries werden zusätzlich in einem kleinen globalen Orphan-Index gespeichert. Die kurzen Laufzeit-Retries bleiben erhalten; darüber hinaus werden offene Orphans beim nächsten Lüftungsassistent-Start und in der täglichen Recorder-Maintenance erneut verarbeitet. Damit überlebt der Retry auch einen HA-Neustart, sobald die Integration wieder aktiv ist.
- **Remote-Duplikat-Log wird dedupliziert:** Eine bereits erkannte Legacy-Doppelkonfiguration derselben stabilen Home-Assistant-Instanz-ID erzeugt pro Lauf nur noch eine Warnung pro Duplikatgruppe statt bei jedem 30-Sekunden-Poll erneut.

### CO₂-Raumansicht und letzte Release-Härtungen
- **Laufende CO₂-Sitzungen bleiben auch in der normalen Raumansicht sichtbar:** Die separate Raumampel kann eine aktive, von der Engine weiterhin auf `keep_open` gehaltene CO₂-Lüftung nicht mehr zu **Grün / „Lüften kann beendet werden“** herunterstufen. Das gilt ausdrücklich auch für den realen Schwellenfall **1440 → 1399 ppm** sowie unabhängig davon, ob die Raumfeuchte unauffällig, leicht erhöht oder bereits selbst orange ist.
- Während der fünfminütigen Mindestphase zeigt die Raumansicht mindestens **Orange / „Weiter lüften“**, sofern die eigentliche Engine weiter offen halten will. Nach der Mindestphase bleibt eine laufende Session oberhalb des Zielbereichs ebenfalls Orange.
- In echter Zielnähe wird die aktive CO₂-Sitzung bei ansonsten neutralen/guten Außenbedingungen als **Gelb / „Noch kurz weiterlüften“** dargestellt. Erst der hysterese- und zeitbestätigte Abschluss (`co2_finish_ready` / `lueftung_fertig`) darf die Session zu **Grün / „Lüften kann beendet werden“** machen. Reale aktuelle Außen-Nachteile und harte Safety-Sperren bleiben weiterhin autoritativ und werden nicht von der Session überstimmt.
- Die explizite CO₂-Session wird zusätzlich über neutrale gelbe Zwischenzustände hinweg erhalten; neutrale Raum-/Feuchtebewertungen dürfen eine noch nicht abgeschlossene Session nicht allein beenden.
- Neue Regressionstests prüfen die komplette Raumkarten-Semantik für **1440 → 1399**, Mindestzeit, Zeit nach Minute 5, verschiedene Feuchtebereiche, Zielnähe 899/880/850/840 ppm und den tatsächlich bestätigten Abschluss.
- **Recorder-Cleanup bei vollständig gelöschter Instanz:** Scheitert der letzte `recorder.purge_entities`-Aufruf beim Entfernen eines ConfigEntry, bleibt die kompakte Retry-Liste erhalten und wird über eine Home-Assistant-eigene, vom bereits gelöschten ConfigEntry unabhängige Retry-Kette erneut versucht. Nach Erfolg wird der Store entfernt.
- **Bestehende Remote-Doppelkonfigurationen werden nachträglich erkannt:** Sobald ältere MagicDNS-/Tailscale-IP-Einträge dieselbe stabile Home-Assistant-Instanz-ID gelernt haben, wird der älteste Eintrag deterministisch als kanonisch behandelt, die Raumauswahl zusammengeführt und der zusätzliche Poller beim nächsten Setup/Reload abgewiesen. Neue Doppelkonfigurationen bleiben bereits im Config Flow blockiert.
- Zusätzliche Regressionstests decken Recorder-Retry nach kompletter Entry-Entfernung und die rückwirkende Remote-Duplikaterkennung über die stabile Server-ID ab.

### Kompakte Raumkarte mit dauerhaftem Detailmodus
- Die normale **Lüftungsassistent-Raumkarte** startet kompakt: Statusfarbe, Raumname und die eigentliche Handlungsempfehlung wie **„Jetzt lüften“**, **„Geschlossen lassen“** oder **„Aktuell kein Lüftungsgrund“** bleiben immer sichtbar. Ein Info-Schalter blendet die bisherigen Detailbereiche ein bzw. wieder aus.
- Die gewählte Ansicht wird pro Browser/Endgerät über einen stabilen `localStorage`-Schlüssel der Raum-Entity gespeichert. Home-Assistant-Neustarts, Config-Reloads und Integrationsupdates ändern die manuelle Wahl daher nicht; ein anderes Endgerät darf unabhängig davon seine eigene Ansicht verwenden.
- **Nachtlüftung bleibt auch im kompakten Zustand sichtbar.** Eine vorhandene Nachtstrategie verschwindet also nicht nur deshalb, weil Messwerte und normale Begründungen eingeklappt sind.
- Eine aktive **harte Fenstersperre** ist ebenfalls eine Ausnahme: Sperrgrund und **„Warum?“** bleiben in der kompakten Ansicht sichtbar, ohne den gespeicherten Kompakt-/Groß-Zustand zu überschreiben. Nach Ende der Sperre kehrt die Karte automatisch zur vorher gewählten Darstellung zurück.
- Eine reine **Entwarnung** wird dagegen nur in der ausgeklappten Detailansicht gezeigt. Gleiches gilt im Normalfall für Begründung, Lüftungsdauer, Messwerte, letzte Lüftung und Diagnosehinweise.
- Raumkarten, die bewusst aus der **Übersicht** in einem Dialog geöffnet werden, erscheinen weiterhin direkt vollständig ausgeklappt.

### Release-Audit-Hardening
- **Amtliche Warnungen beim Start:** Raumbenachrichtigungen werden erst freigegeben, nachdem alle Raum-Coordinatoren ihren ersten Snapshot besitzen. Die Sperre greift ausdrücklich auch im echten `async_config_entry_first_refresh()`-/`_async_update_data()`-Pfad; ein später gestarteter Raum mit offenem Fenster kann dadurch nicht mehr kurzzeitig als „nicht offen“ fehlen und eine falsche Meldung „alle Fenster/Türen geschlossen“ auslösen.
- **NINA-Detailcache:** `nina.get_details` wird weiterhin nach fünf Minuten neu abgefragt. Scheitert dieser Refresh kurzzeitig, darf **nur dieselbe weiterhin aktive Warning-ID** ihre zuletzt erfolgreich geladenen Safety-Details noch begrenzt bis maximal 60 Minuten verwenden. Eine neue Warning-ID erbt niemals alte Schutztexte, inaktive Slots werden sofort entfernt und nach der harten Stale-Grenze wird ebenfalls verworfen. Damit gibt es weder unbegrenzt alte Warntexte noch ein Fail-open nach einem einzelnen Providerfehler.
...
- **Persistierte Zeitstempel:** Wiederhergestellte Zeitstempel aus der Zukunft werden in Decision-Memory, CO₂-Grace, Air-Quality-Memory, Schimmel-/Airing-Speichern und kurzen Hysterese-Timern nicht mehr als frisch akzeptiert.
- **DST/Winterzeit:** Night-Memory-Restore vergleicht Start/Ende auf einer absoluten UTC-Timeline und unterscheidet die beiden lokalen `02:xx`-Stunden beim DST-Fold korrekt.
- **Recorder-Retention:** Der domainweite tägliche Purge-Task gehört nicht mehr zufällig einem einzelnen ConfigEntry, sondern Home Assistant selbst. Solange noch irgendeine Lüftungsassistent-Instanz geladen ist, kann das Entladen einer anderen den gemeinsamen Lauf nicht abbrechen; beim letzten Unload wird er weiterhin sauber beendet.
- **Gelöschte HA-Areas:** Verschwindet eine gespeicherte Area außerhalb des Lüftungsassistenten, wird der stale Integrations-Link freigegeben, ohne eine inzwischen manuell gesetzte Device-Area zu löschen. Ein späteres Reconfigure interpretiert die nicht mehr darstellbare alte Area nicht als bewusste Löschaktion.
- **Warn-Fingerprint bei Reload:** Der assistentenweite Fingerprint einer laufenden amtlichen Warnung bleibt über normale ConfigEntry-Reloads erhalten und wird erst bei echter Entfernung der Integration verworfen. Ein bloßes Reconfigure erzeugt dadurch keine identische Warn-Push-Nachricht erneut; nach einem echten HA-Neustart darf weiterhin neu gewarnt werden.

### Setup-, Retry- und Provider-Hardening
- **Runtime-Factorys veröffentlichen erst nach erfolgreicher Initialisierung:** Remote-, Outside- und Raum-Coordinator sowie Airing-, CO₂-, Mold- und Air-Quality-Tracker werden erst nach einem vollständig erfolgreichen Start in `hass.data` abgelegt. Ein fehlgeschlagener erster Refresh/Store-Load kann bei einem späteren Home-Assistant-Retry damit kein halb initialisiertes oder bereits heruntergefahrenes Objekt wiederverwenden. Airing/CO₂ räumen bei spät scheiternder Initialisierung bereits gesetzte Listener/Timer sofort wieder auf.
- **Zentraler Setup-Rollback:** Scheitert ein späterer Schritt des gesamten ConfigEntry-Setups, werden bereits veröffentlichte Runtime-Komponenten wieder aus den eigenen Caches entfernt und heruntergefahren. Auch ein nur teilweise erfolgreiches Platform-Forwarding wird im Fehlerpfad wieder entladen.
- **Remote-Reauth/Reconfigure lädt zuverlässig neu:** Remote-Einträge verwenden bewusst keinen ConfigEntry-Update-Listener mehr. Erfolgreicher Reauth und Remote-Reconfigure aktualisieren den Eintrag über Home Assistants Config-Flow-Reload-Helper und lösen genau einen Reload aus. Damit funktioniert ein neuer Token auch dann, wenn bereits der allererste Setup-Versuch wegen Authentifizierung scheiterte, ohne Listener/Flow-Doppel-Reload.
- **NINA-Provider-Ausfall bleibt fail-safe:** Ein explizites `off` bleibt eine sofortige autoritative Entwarnung. Werden NINA-Warnsensoren dagegen vorübergehend `unknown`/`unavailable`, darf ausschließlich die zuletzt sicher aktive **Danger-/Fenster-schließen-Anweisung derselben Warning-ID** noch innerhalb der bestehenden 60-Minuten-Stale-Grenze weiterwirken. Neue Warning-IDs erben keine alten Safety-Texte und stale Entwarnungen werden nicht konserviert.
- **Frontend-Lokalisierung retryt kontrolliert:** Kurzzeitige WebSocket-Fehler lassen Karten nicht dauerhaft auf Fallbacktext stehen. Localization-Requests werden mit exponentiellem Backoff erneut angestoßen; mehrere wartende Overview-Räume bündeln ihre fertigen Texte auf einen Render pro Microtask statt unmittelbar viele Voll-Render hintereinander auszulösen.
- **Remote-Access-Lifecycle:** Die kurzfristigen 95-Sekunden-Remote-Access-Timer werden beim Unload/Remove eines ConfigEntry sofort gecancelt und aus dem Store entfernt, statt erst später selbst auszulaufen.
- Neue Regressionstests decken fehlgeschlagene Remote-Erstinitialisierung, Remote-Reauth ohne vorherigen Update-Listener, publish-after-init für alle Runtime-Factorys, zentralen Setup-Rollback/partielles Platform-Forwarding, NINA-`unavailable` vs. autoritatives `off`, Warning-ID-Wechsel, Localization-Retry/-Render-Coalescing und Remote-Access-Timer-Cleanup ab.

### Persistente Safety-, Notification- und Remote-Identity-Härtung
- **Providerübergreifender persistenter Hard-Safety-State:** Bereits sicher bestätigte harte Fenster-/Türsperren werden jetzt als kleiner normalisierter Zustand pro Lüftungsassistent-Instanz persistiert. Gespeichert werden nur Quelle, konkrete Warn-Entity/Warning-ID, semantischer Grund und eine harte Ablaufzeit – keine kompletten NINA-/DWD-Rohpayloads. Dadurch überlebt eine bekannte Schutzanweisung normale ConfigEntry-Reloads und auch einen Home-Assistant-Neustart.
- **ACTIVE/CLEAR/UNKNOWN statt „unavailable = keine Warnung“:** NINA, DWD, generische Warnprovider und harte Live-Wettergefahren verwenden dieselbe fail-safe Grundregel. Ein explizites `off`/CLEAR hebt die Sperre sofort auf; `unknown`/`unavailable` darf ausschließlich eine zuvor bestätigte harte Sperre derselben Quelle für maximal 60 Minuten fortführen. Eine neue Warning-ID oder ein neuer Warnslot erbt niemals blind den alten Zustand.
- **Konkrete Warn-Entity gebunden:** Der persistierte Warnzustand merkt sich zusätzlich genau die Provider-Entity, welche die harte Sperre ausgelöst hat. Dadurch kann eine neue Warnung in einem anderen NINA-/DWD-Slot nicht versehentlich alte Schutzinformationen übernehmen, während der Ausfall genau des bisherigen Warnslots weiterhin begrenzt fail-safe behandelt wird.
- **Notification-Service wirklich abgewartet:** Safety-Pushes verwenden für `notify.send_message` jetzt `blocking=True`. Ein Service-/Validierungsfehler kann damit nicht mehr als erfolgreicher Versand markiert werden; der Notification-Fingerprint wird erst nach erfolgreicher HA-Serviceausführung gespeichert.
- **Fingerprint gilt nur für eine durchgehende Gefahr:** Verschwindet eine relevante Gefahr, wird ihr Hazard-Fingerprint sofort gelöscht – unabhängig davon, ob ein Fenster weiterhin offen ist. Tritt dieselbe Gefahr später erneut auf, kann wieder eine Warnung gesendet werden.
- **Stabilere DWD-Warnungsidentität:** Fehlt eine echte Provider-ID, wird die Fallback-ID aus semantischen Warnfeldern wie Typ/Name, Beginn/Ende und Headline gehasht statt aus der veränderlichen Listenposition `warning_1`, `warning_2`, … gebildet. Sortieränderungen erzeugen dadurch keine künstlich neue Warnung.
- **Recorder-Purge behält fehlgeschlagene IDs:** `recorder.purge_entities` wird auf Serviceebene abgewartet. Schlägt der Aufruf fehl, bleiben entfernte Entity-IDs im eigenen Retry-Index statt sofort vergessen zu werden. Der generische Store-Cleanup löscht diesen spezialisierten Retry-Index deshalb nicht mehr nachträglich.
- **Stabile Remote-Home-Assistant-ID:** Der Remote-Snapshot liefert optional Home Assistants stabile Core-UUID mit. Remote-Einträge speichern diese ID nach erfolgreicher Verbindung und verwenden sie zusätzlich zur Host/Port-Normalisierung für die Duplikaterkennung. Dadurch wird dieselbe Gegenstelle auch über MagicDNS-Name und direkte Tailscale-IP als dieselbe HA-Instanz erkannt. Ältere Remote-Versionen ohne diese additive ID bleiben kompatibel.
- Neue Regressionstests decken Safety-Persistenz über Reload/Neustart, NINA/DWD/generische Provider bei `unavailable`, autoritatives `off`, Warning-ID-Wechsel, Notification-Servicefehler/Fingerprint-Episoden, stabile DWD-IDs, Recorder-Purge-Retry und Remote-Duplikate über die stabile HA-Instanz-ID ab.

### Optionale Home-Assistant-Raumzuordnung
- Räume können beim Anlegen oder späteren Konfigurieren optional einem bereits vorhandenen **Home-Assistant-Bereich (Area)** zugeordnet werden. Dafür verwendet der Config Flow den nativen Home-Assistant-`AreaSelector`.
- Der freie Lüftungsassistent-Raumname bleibt erhalten. **Bereich und eigener Name sind unabhängig voneinander**: Mit eigenem Namen behält der Raum dieses Label; ohne eigenen Namen übernimmt ein neu angelegter Raum den aktuellen Namen des ausgewählten Home-Assistant-Bereichs.
- Das vom Lüftungsassistent erzeugte Raumgerät wird nach dem Plattform-Setup über die Home-Assistant-Device-Registry dem gespeicherten `area_id` zugeordnet. Dadurch landen seine Entities sauber im gewählten Home-Assistant-Bereich, ohne Lovelace-/Dashboard-Konfigurationen automatisch zu verändern.
- Wird eine zuvor über den Lüftungsassistent konfigurierte Area im Reconfigure bewusst entfernt, wird auch die Gerätezuordnung wieder gelöst. **Bestehende Räume aus älteren Versionen besitzen dagegen keinen `area_id`-Marker und bleiben unangetastet**, damit ein Update keine bereits manuell in Home Assistant gesetzte Area löscht.
- Wird ein gespeicherter Home-Assistant-Bereich außerhalb der Integration gelöscht, wird kein ungültiger Bereich erneut auf das Gerät geschrieben; der Raum bleibt weiterhin funktionsfähig und kann neu zugeordnet werden.
- Die Device-Registry-Synchronisierung nutzt unter HA 2026.8+ die neue config-entry-scoped Identifier-Suche und enthält für die weiterhin unterstützten HA-2026.6/2026.7-Versionen einen gezielten kompatiblen Fallback.

### Oberfläche, Dokumentation und Tests
- Raum-Setup und Reconfigure enthalten neue Beschriftungen und Hilfetexte für **Deutsch, Englisch und Türkisch**.
- Validierung verlangt jetzt mindestens **einen freien Raumnamen oder eine gültige Home-Assistant-Area**; doppelte aufgelöste Raumtitel innerhalb derselben Lüftungsassistent-Instanz bleiben weiterhin gesperrt.
- README und Regressionstests wurden um Area-Auswahl, Namens-Fallback, Legacy-Verhalten, Entfernen der Zuordnung und Device-Registry-Sync erweitert.
- Manifest, gemeinsame `INTEGRATION_VERSION`, Frontend-Cache-Buster-Test und Bugreport-Vorlage stehen auf **0.9.4**.
